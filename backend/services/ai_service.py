...
6. vibe_profile: 一段话（2-3句，60-100字），更完整地描述 TA 是个怎样的人、在社交/合作上想要什么，第二人称（"你..."）
7. system_tags: 3-5个偏向「合作意图/角色定位」的系统标签，格式 "#xxx"（与 taste_tags 不重复，例如 #找长期合作者 #偏好直接沟通 #开放 CoffeeChat）

注意：
- 一切内容都要结合用户的具体回答来写，不要套模板
- soul_slices 和 vibe_profile 要用第二人称（"你..."）
...
AGENT_SYSTEM_PROMPT = """你是 HackerLink 的 AI 助手，一个友好、聪明、乐于助人的对话伙伴。
你帮助黑客松的参与者梳理想法、寻找搭档、打磨 demo，也可以闲聊。
回答要简洁自然、有温度。默认用中文回复，除非用户用其他语言提问。"""
...
async def generate_twin(
    ...
    try:
        prompt = TWIN_GENERATION_PROMPT.format(
            display_name=display_name or "未提供",
...
async def agent_chat(
    ...
    """Chat directly with the GLM-backed agent.

    `messages` is the conversation history as a list of
    {"role": "user" | "assistant", "content": str}. A system prompt is
    prepended automatically.
    """
    if not _get_glm_api_key():
        raise ValueError(
            "GLM_API_KEY 未配置。请在 backend/.env 中设置 GLM_API_KEY 后重启服务。"
        )
    ...
    return await _glm_chat_messages(glm_messages, max_tokens=max_tokens)


async def judge_solved(prompt: str) -> dict:
    """Ask the model whether the user's issue is resolved. Returns
    {"solved": bool, "reason": str}. Never raises; defaults to not-solved."""
    if not _get_glm_api_key():
        return {"solved": False, "reason": "未配置模型，无法自动确认"}
    try:
        text = await _glm_chat(prompt, max_tokens=200)
        ...
        return {"solved": False, "reason": f"判定失败：{exc}"}
...
async def _glm_chat(prompt: str, max_tokens: int) -> str:
    return await _glm_chat_messages([{"role": "user", "content": prompt}], max_tokens)


async def _glm_chat_messages(messages: list[dict], max_tokens: int) -> str:
    return await asyncio.to_thread(_glm_chat_messages_sync, messages, max_tokens)


def _glm_chat_messages_sync(messages: list[dict], max_tokens: int) -> str:
    settings = get_settings()
